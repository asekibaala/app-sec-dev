...
class ScanRequestValidatorTests(unittest.TestCase):
    def test_normalises_scheme_path_and_case(self):
        request = ScanRequest(
            scan_name="External perimeter review",
            domain="  HTTPS://Example.COM/path?q=1#frag ",
        )
        self.assertEqual(request.domain, "example.com")
        self.assertEqual(request.scan_name, "External perimeter review")

    def test_rejects_empty_scan_name(self):
        with self.assertRaises(ValidationError) as context:
            ScanRequest(scan_name="   ", domain="example.com")

        self.assertIn("Scan name cannot be empty", str(context.exception))

    def test_rejects_private_ip(self):
        with self.assertRaises(ValidationError) as context:
            ScanRequest(scan_name="Private IP test", domain="127.0.0.1")
